...
export default function SiswaPage() {
  ...
  const fetchSiswa = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("*")
      .order("nama");
    if (error) toast.error("Gagal memuat data siswa");
    else setSiswaList(data || []);
    setLoading(false);
  }, [supabase]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingSiswa) {
      const { error } = await supabase.from("siswa").update(form).eq("id", editingSiswa.id);
      if (error) toast.error("Gagal mengupdate siswa");
      else toast.success("Siswa berhasil diupdate");
    } else {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
      const { data: userData } = await supabase.from("users").select("madrasah_id").eq("id", user.id).single();
      const { error } = await supabase.from("siswa").insert({ ...form, madrasah_id: userData?.madrasah_id });
      if (error) toast.error("Gagal menambah siswa");
      else toast.success("Siswa berhasil ditambahkan");
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    else { toast.success("Siswa berhasil dihapus"); fetchSiswa(); }
  };
