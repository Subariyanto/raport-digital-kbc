...
export default function GuruPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const list = demoStore.getGuru();
    if (editingGuru) {
      const updated = list.map(g => g.id === editingGuru.id ? { ...g, ...form } : g);
      demoStore.setGuru(updated);
      toast.success("Guru berhasil diupdate");
    } else {
      list.push({ ...form, id: demoStore.generateId(), madrasah_id: "11111111-1111-1111-1111-111111111111", user_id: null, created_at: "", updated_at: "" });
      demoStore.setGuru(list);
  ...
  const handleDelete = (id: string) => {
    ...
    const list = demoStore.getGuru().filter(g => g.id !== id);
    demoStore.setGuru(list);
