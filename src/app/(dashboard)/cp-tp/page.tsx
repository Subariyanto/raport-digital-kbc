...
import { MataPelajaran, CapaianPembelajaran, TujuanPembelajaran } from "@/lib/types";
...
export default function CpTpPage() {
  ...
  const fetchCpTp = useCallback(async () => {
    ...
    setCpList(data || []);
    setLoading(false);
  }, [selectedMapel, selectedFase, supabase]);
  ...
  const handleSaveCp = async (e: React.FormEvent) => {
    ...
    if (editingCp) {
      const { error } = await supabase.from("capaian_pembelajaran").update({ deskripsi: cpForm.deskripsi }).eq("id", editingCp.id);
      if (error) toast.error("Gagal mengupdate CP");
      else toast.success("CP berhasil diupdate");
    } else {
      const { error } = await supabase.from("capaian_pembelajaran").insert({
        mapel_id: selectedMapel, fase: selectedFase, jenjang: null, deskripsi: cpForm.deskripsi,
      });
      if (error) toast.error("Gagal menambah CP");
      else toast.success("CP berhasil ditambahkan");
    }
    setShowCpForm(false);
    setEditingCp(null);
    setCpForm({ deskripsi: "" });
  ...
  const handleDeleteCp = async (id: string) => {
    ...
    const { error } = await supabase.from("capaian_pembelajaran").delete().eq("id", id);
    if (error) toast.error("Gagal menghapus CP");
    else { toast.success("CP berhasil dihapus"); fetchCpTp(); }
  };
  ...
  const handleSaveTp = async (e: React.FormEvent) => {
    ...
    if (editingTp) {
      const { error } = await supabase.from("tujuan_pembelajaran").update({ kode: tpForm.kode, deskripsi: tpForm.deskripsi, urutan: tpForm.urutan }).eq("id", editingTp.id);
      if (error) toast.error("Gagal mengupdate TP");
      else toast.success("TP berhasil diupdate");
    } else {
      const { error } = await supabase.from("tujuan_pembelajaran").insert({
        cp_id: tpForm.cp_id, kode: tpForm.kode, deskripsi: tpForm.deskripsi, urutan: tpForm.urutan,
      });
      if (error) toast.error("Gagal menambah TP");
      else toast.success("TP berhasil ditambahkan");
    }
    setShowTpForm(false);
    setEditingTp(null);
  ...
  const handleDeleteTp = async (id: string) => {
    ...
    if (error) toast.error("Gagal menghapus TP");
    else { toast.success("TP berhasil dihapus"); fetchCpTp(); }
  };
              ...
              {expandedCp.includes(cp.id) && (
                <div className="border-t px-4 pb-4">
                  <div className="mt-3 space-y-2">
                    {(cp.tujuan_pembelajaran || []).sort((a, b) => a.urutan - b.urutan).map((tp) => (
